...
class CompositionalHMMDataset(Dataset):
    ...
    def _make_index_to_latent(self):
        ...
        filtered_strings = [list(s) for s in all_binary_strings if s.count('1') >= min_active_latents]
    ...
    def _generate_cycle_composition_graph(self):
        ...
        for _ in range(self.cfg.n_latents):
            ...
            if len(used_nodes) != 0:
                nodes.update(
                    np.random.choice(list(used_nodes), min(self.cfg.n_overlap, len(used_nodes)), replace=False)
                )
    ...
    def __getitem__(self, index: int, n_step: int = 10):

        model = hmm.CategoricalHMM(n_components=self.cfg.n_states + 1, n_features=self.cfg.n_states + 1)
