# from https://github.com/GFNOrg/gfn-diffusion/blob/main/vae/models/gfn.py
...
class GFN(nn.Module):
    def __init__(
        ...
        self.joint_model = JointPolicyVAE(
            dim, s_emb_dim, t_dim, hidden_dim, 2 * dim, joint_layers, zero_init
        )
        if learn_pb:
            self.back_model = JointPolicyVAE(
                dim, s_emb_dim, t_dim, hidden_dim, 2 * dim, joint_layers, zero_init
            )
        self.pb_scale_range = pb_scale_range

        if self.partial_energy:
            self.flow_model = FlowModelPartialEnergyVAE(
                dim, s_emb_dim, t_dim, hidden_dim, 1, joint_layers
            )
        else:
            self.flow_model = FlowModelVAE(
                dim, s_emb_dim, t_dim, hidden_dim, 1, joint_layers
            )

        if self.langevin_scaling_per_dimension:
            self.langevin_scaling_model = LangevinScalingModelVAE(
                s_emb_dim, t_dim, hidden_dim, dim, lgv_layers, zero_init
            )
        else:
            self.langevin_scaling_model = LangevinScalingModelVAE(
                s_emb_dim, t_dim, hidden_dim, 1, lgv_layers, zero_init
            )
        ...
        if self.langevin:
            ...
            with torch.enable_grad():
                if condition is not None:
                    grad_log_r = torch.autograd.grad(log_r(s, condition).sum(), s)[
                        0
                    ].detach()
                else:
    ...
    def get_trajectory_fwd(self, s, exploration_std, log_r, condition=None):
        ...
        for i in range(self.trajectory_length):
            ...
            if self.partial_energy:
                ...
                logf[:, i] += (1 - i * self.dt) * log_p_ref + i * self.dt * log_r(
                    s, condition
                )
    ...
    def get_trajectory_bwd(self, s, exploration_std, log_r, condition=None):
        ...
        for i in range(self.trajectory_length):
            ...
            pfs, flow = self.predict_next_state(
                s_, (1.0 - (i + 1) * self.dt), log_r, condition
            )
            pf_mean, pflogvars = self.split_params(pfs)
